...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        const e = array[index];
        // console.log(e.href)
        if (e.href.includes("/songs/") && e.href.split("/").slice(-2)[0]!=="") {
            console.log(e.href);
...
async function main() {
    ...
    function previousButtonClickHandler() {
        index = songs.indexOf(audiog.src.split("/").slice(-1)[0]);
        if ((index - 1) >= 0) {
            if (convertSecondsToTime(audiog.currentTime) > "0:03") {  // Corrected this line
    ...
    audiog.addEventListener('volumechange', () => {
        if (audiog.volume == 0) {
            document.querySelector(".volume-icon").src = "assets/mute.svg";
        }
        else {
